    ...
    def validate_against_rules(
        ...
        for rule_id in rule_nodes:
            ...
            if "rule_no_skip_stages" in rule_id and "skip" in action_lower:
                violations.append(
                    {"rule": rule_id, "severity": "error", "message": node.content}
                )

            if "rule_must_test" in rule_id and (
                "implement" in action_lower or "code" in action_lower
            ):
                applicable_rules.append(
                    {"rule": rule_id, "type": "requirement", "content": node.content}
                )
                ...
                suggestions.append(
                    {
                        ...
                        "command": "wf advance " + current.feature_id,
                    }
                )
