...
class TestCmdValidate:
    ...
    def test_cmd_validate_shows_recommendations(self, capsys):
        ...
        assert ("RECOMMENDATIONS:" in captured.out) or ("VALIDATION REPORT" in captured.out)
...
class TestCmdStats:
    ...
    def test_cmd_stats_shows_average_degree(self, capsys):
        ...
        assert ("Average Degree:" in captured.out) or ("GRAPH STATISTICS" in captured.out)
...
class TestCmdServer:
    ...
    def test_cmd_server_logs_startup_info(self, mock_run, capsys):
        ...
        captured = capsys.readouterr()
        assert "Starting server" in captured.err or "Admin panel" in captured.err or mock_run.called
...
class TestMain:
    ...
    def test_main_executes_export_command(self, tmp_path):
        ...
        output_file = tmp_path / "export.json"
        with patch.object(sys, 'argv', ['cli', 'export', '--agent', 'default', '--output', str(output_file)]):
            result = main()
    ...
    def test_main_handles_command_errors(self):
        ...
        with patch.object(sys, 'argv', ['cli', 'export', '--agent', 'test']):
            with patch('ai_dev_graph.cli.cmd_export', side_effect=Exception("Test error")):
                result = main()
