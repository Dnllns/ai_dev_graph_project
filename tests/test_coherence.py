...
def graph_data():
    ...
    if not GRAPH_PATH.exists():
        pytest.fail(f"El artefacto {GRAPH_PATH} no existe. Ejecuta 'python -m ai_dev_graph.init_meta_graph'")
...
def test_graph_is_connected(graph_data):
    ...
    if len(g.nodes) > 0:
        ...
        assert nx.is_weakly_connected(g), "El grafo de conocimiento está fragmentado (hay nodos inalcanzables)."
...
def test_taxonomy_compliance(graph_data):
    ...
    for node_id, attrs in g.nodes(data=True):
        ...
        assert node_type in valid_types, f"El nodo {node_id} tiene un tipo inválido: {node_type}"
        ...
        for start, end in emoji_ranges:
            for char in content:
                if start <= ord(char) <= end:
                    pytest.fail(f"VIOLACIÓN DE POLÍTICA RAW: El nodo '{node_id}' contiene emoji/símbolo '{char}'.")